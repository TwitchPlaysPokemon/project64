...
void CloseCpu (void) {
	...
	EnableMenuItem(hMainMenu,ID_FILE_ENDEMULATION,MFS_DISABLED|MF_BYCOMMAND);
	EnableMenuItem(hMainMenu,ID_OPTIONS_FULLSCREEN,MFS_DISABLED|MF_BYCOMMAND);
	EnableMenuItem(hMainMenu,ID_CPU_RESET,MFS_DISABLED|MF_BYCOMMAND);
	EnableMenuItem(hMainMenu,ID_CPU_PAUSE,MFS_DISABLED|MF_BYCOMMAND);
	EnableMenuItem(hMainMenu,ID_SYSTEM_GENERATEBITMAP,MFS_DISABLED|MF_BYCOMMAND);
	EnableMenuItem(hMainMenu,ID_CPU_SAVE,MFS_DISABLED|MF_BYCOMMAND);
	EnableMenuItem(hMainMenu,ID_CPU_SAVEAS,MFS_DISABLED|MF_BYCOMMAND);
	EnableMenuItem(hMainMenu,ID_CPU_RESTORE,MFS_DISABLED|MF_BYCOMMAND);
	EnableMenuItem(hMainMenu,ID_CPU_LOAD,MFS_DISABLED|MF_BYCOMMAND);
	hSubMenu = GetSubMenu(hMainMenu,1);
	EnableMenuItem(hSubMenu,9,MFS_DISABLED|MF_BYPOSITION);
	...
	sprintf(StatusString,"Emulation ended");
...
void DoSomething ( void ) {
	...
	if (CPU_Action.Pause) {
		...
		if (CPU_Action.Pause) {
			...
			MenuInfo.cch = 256;
			SetMenuItemInfo(hMainMenu,ID_CPU_PAUSE,FALSE,&MenuInfo);
...
InterruptsDisabled:
	...
	CurrentPercent = 0;
	DisplayFPS();
	DisplayError("In a permanent loop that can not be exited\n\nEmulation will now stop");
	ExitThread(0);
}
...
BOOL Machine_SaveState(void) {
	...
	if (AutoZip) {
		...
		if (hSaveFile == INVALID_HANDLE_VALUE) {
			...
			case ERROR_PATH_NOT_FOUND:
				...
				if (hSaveFile == INVALID_HANDLE_VALUE) {
					DisplayError("Failed to open Save File");
					return TRUE;
				}
				break;
			default:
	...
	strcpy(SaveAsFileName,"");
	strcpy(LoadFileName,"");
	sprintf(String,"Saved current state to %s",FileName);
	SendMessage( hStatusWnd, SB_SETTEXT, 0, (LPARAM)String );
	return TRUE;
}
...
void PauseCpu (void) {
	...
	if (CPU_Paused || CPU_Action.Pause) {
		...
		if (CPU_Action.Pause) {
			...
			CPU_Paused = FALSE;
			ManualPaused = FALSE;
			SendMessage( hStatusWnd, SB_SETTEXT, 0, (LPARAM)"CPU Resumed" );
			ReleaseMutex(hPauseMutex);
			return;
		}
		ResumeThread(hCPU);
...
void RefreshScreen (void ){ 
	...
	memcpy(Label,OldProfilingLabel,sizeof(Label));
	...
	if (ShowCPUPer) { StartTimer("r4300i Running"); }
}
...
//LogMessage("Alist");
			...
			if (Profiling || DisplayCPUPer) {
				...
				if (IndvidualBlock && !DisplayCPUPer) {
					...
					default: StartTimer("RSP: Unkown"); break;
					}
				}
				memcpy(Label,OldProfilingLabel,sizeof(Label));
				DoRspCycles(100);
...
void StartEmulation ( void ) {
	...
	EnableMenuItem(hMainMenu,ID_FILE_STARTEMULATION,MFS_DISABLED|MF_BYCOMMAND);
	EnableMenuItem(hMainMenu,ID_FILE_ENDEMULATION,MFS_ENABLED|MF_BYCOMMAND);
	EnableMenuItem(hMainMenu,ID_OPTIONS_FULLSCREEN,MFS_ENABLED|MF_BYCOMMAND);
	EnableMenuItem(hMainMenu,ID_CPU_RESET,MFS_ENABLED|MF_BYCOMMAND);
	EnableMenuItem(hMainMenu,ID_CPU_PAUSE,MFS_ENABLED|MF_BYCOMMAND);
	if (CaptureScreen != NULL) { EnableMenuItem(hMainMenu,ID_SYSTEM_GENERATEBITMAP,MFS_ENABLED|MF_BYCOMMAND); }
	EnableMenuItem(hMainMenu,ID_CPU_SAVE,MFS_ENABLED|MF_BYCOMMAND);
	EnableMenuItem(hMainMenu,ID_CPU_SAVEAS,MFS_ENABLED|MF_BYCOMMAND);
	EnableMenuItem(hMainMenu,ID_CPU_RESTORE,MFS_ENABLED|MF_BYCOMMAND);
	EnableMenuItem(hMainMenu,ID_CPU_LOAD,MFS_ENABLED|MF_BYCOMMAND);
	hSubMenu = GetSubMenu(hMainMenu,1);
	EnableMenuItem(hSubMenu,11,MFS_ENABLED|MF_BYPOSITION);
	...
	sprintf(StatusString,"Emulation started");
	SendMessage( hStatusWnd, SB_SETTEXT, 0, (LPARAM)StatusString );
}
