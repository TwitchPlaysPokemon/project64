...
void SetupPlugins (HWND hWnd) {
	...
	EnableMenuItem(hMenu,ID_FILE_OPEN_ROM,MFS_GRAYED|MF_BYCOMMAND);		
	EnableMenuItem(hMenu,ID_OPTIONS_CONFIG_GFX,MFS_GRAYED|MF_BYCOMMAND);		
	EnableMenuItem(hMenu,ID_OPTIONS_CONFIG_RSP,MFS_GRAYED|MF_BYCOMMAND);		
	EnableMenuItem(hMenu,ID_OPTIONS_CONFIG_CONTROL,MFS_GRAYED|MF_BYCOMMAND);		
	...
	if (!LoadRSPDll(RspDLL)) { 
		DisplayError("Could Not load RSP plugin ...");
		return;
	} else { 
...
//		Controllers[0].Plugin  = PLUGIN_RUMBLE_PAK;
	...
	if (GFXDllConfig) { EnableMenuItem(hMenu,ID_OPTIONS_CONFIG_GFX,MFS_ENABLED|MF_BYCOMMAND); }
	if (RSPDllConfig) { EnableMenuItem(hMenu,ID_OPTIONS_CONFIG_RSP,MFS_ENABLED|MF_BYCOMMAND); }
	if (AiDllConfig) { EnableMenuItem(hMenu,ID_OPTIONS_CONFIG_AUDIO,MFS_ENABLED|MF_BYCOMMAND); }
	if (ContConfig) { EnableMenuItem(hMenu,ID_OPTIONS_CONFIG_CONTROL,MFS_ENABLED|MF_BYCOMMAND); }
...
void SetupPluginScreen (HWND hDlg) {
	...
	_GetPrivateProfileString(Language,"AboutButton","About",String,sizeof(String),LangFile);
#ifndef EXTERNAL_RELEASE
	SetDlgItemText(hDlg,RSP_ABOUT,String);
#endif
	SetDlgItemText(hDlg,GFX_ABOUT,String);
	SetDlgItemText(hDlg,AUDIO_ABOUT,String);
	SetDlgItemText(hDlg,CONT_ABOUT,String);
#ifndef EXTERNAL_RELEASE
	_GetPrivateProfileString(Language,"PluginType1","Reality Signal Processor",String,sizeof(String),LangFile);
	sprintf(Plugin," %s: ",String);
	SetDlgItemText(hDlg,IDC_RSP_NAME,Plugin);
#endif
	_GetPrivateProfileString(Language,"PluginType2","Graphics",String,sizeof(String),LangFile);
	sprintf(Plugin," %s: ",String);
	SetDlgItemText(hDlg,IDC_GFX_NAME,Plugin);
	_GetPrivateProfileString(Language,"PluginType3","Audio",String,sizeof(String),LangFile);
	sprintf(Plugin," %s: ",String);
	SetDlgItemText(hDlg,IDC_AUDIO_NAME,Plugin);
	_GetPrivateProfileString(Language,"PluginType4","Controller",String,sizeof(String),LangFile);
	sprintf(Plugin," %s: ",String);
	SetDlgItemText(hDlg,IDC_CONT_NAME,Plugin);
	...
	for (;;) {
		...
		if (hLib == NULL) { 
			DisplayError("Failed to load: %s",Plugin); 
		...
		GetDllInfo(&PluginInfo);
		if (!ValidPluginVersion(&PluginInfo) || PluginInfo.MemoryBswaped == FALSE) {
			if (FindNextFile(hFind,&FindData) == 0) { return; }
			continue;
		}
...
void ShutdownPlugins (void) {
	if (IsMenu(RspDebug.hRSPMenu)) {
		HMENU hMenu = GetMenu(hMainWindow);
		DeleteMenu(hMenu,(DWORD)RspDebug.hRSPMenu, MF_BYCOMMAND);		
	}
	if (IsMenu(GFXDebug.hGFXMenu)) {
		HMENU hMenu = GetMenu(hMainWindow);
		DeleteMenu(hMenu,(DWORD)GFXDebug.hGFXMenu, MF_BYCOMMAND);		
	}
