...
BYTE * Compiler4300iBlock(void) {
	...
	//if (Profiling) { StopTimer(TRUE); }	
	if (Profiling) { StartTimer("r4300i Running"); }
...
void _fastcall CreateSectionLinkage (BLOCK_SECTION * Section) {
	...
	__try {
		...
		DisplayError("Unknown memory action in trying to Fill Section Info\n\nEmulation stop");
		ExitThread(0);
	}
...
void _fastcall FillSectionInfo(BLOCK_SECTION * Section) {
	...
	do {
		if (!r4300i_LW_VAddr(Section->CompilePC, &Command.Hex)) {
			DisplayError("Failed to load word %X\n2",Section->CompilePC);
			ExitThread(0);
		} 
		if (SelfModCheck == ModCode_ChangeMemory) {
				...
				case R4300i_COP1_BC_BCTL:
					...
					if (Section->CompilePC == Section->Jump.TargetPC) {
						...
						if (!r4300i_LW_VAddr(Section->CompilePC + 4, &NewCommand.Hex)) {
							DisplayError("Failed to load word 3");
							ExitThread(0);
						}
...
BOOL GenerateX86Code (BLOCK_SECTION * Section, DWORD Test) {
	...
	do {
		__try {
			...
			DisplayError("Unknown memory action in trying to Generate X86 Code\n\nEmulation stop");
			ExitThread(0);
		}
		if (SelfModCheck == ModCode_ChangeMemory) {
...
void StartRecompilerCPU (void ) { 
	...
	if (ModCode_CheckMemoryCache) {
		if (TargetInfo == NULL) {
			TargetInfo = VirtualAlloc(NULL,MaxCodeBlocks * sizeof(TARGET_INFO),MEM_COMMIT|MEM_RESERVE,PAGE_READWRITE);
			if (TargetInfo == NULL) {
				DisplayError("Failed to allocateMemory for Target Info table, emulation stoping.");
				ExitThread(0);
			}
		}
		TargetIndex = 0;
	...
	__try {
		if (SelfModCheck == ModCode_ChangeMemory) {
			...
			for (;;) {
				...
				__try {
					...
					DisplayError("Executing from non maped space");
					ExitThread(0);
				}
				...
				if (Block == NULL) {
					...
					if (TargetIndex == MaxOrigMem) {
						DisplayError("Out of Mem sections");
						return;
					}
					Value = 0x7C7C0000;
					Value += (WORD)(TargetIndex);
		...
		for (;;) {
			...
			if (Block == NULL) {
				...
				if (SelfModCheck == ModCode_CheckMemoryCache) {
					...
					*(JumpTable + (Addr >> 2)) = &TargetInfo[TargetIndex];
					...
					if (TargetIndex == MaxCodeBlocks) {
						DisplayError("Max Code blocks reached");
						ExitThread(0);
					}
				} else {
		...
		DisplayError("Unknown memory action\n\nEmulation stop");
		ExitThread(0);
	}
}
