...
int Allocate_Memory ( void ) {	
	...
	(BYTE *)JumpTable = (BYTE *) VirtualAlloc( NULL, 0x20000000, MEM_RESERVE | MEM_TOP_DOWN, PAGE_READWRITE );
	if( JumpTable == NULL ) {  
	...
	if(RecompCode==NULL) {  
		DisplayError("Not enough memory for RecompCode!");
		return FALSE;
	}
	...
	if (TLB_ReadMap == NULL) {
		DisplayError("Not enough memory for TLB_ReadMap!");
		return FALSE;
	}
	...
	if( DelaySlotTable == NULL ) {  
		DisplayError("Not enough memory for Delay Slot Table!");
		return FALSE;
	}
	...
	if(VirtualAlloc(JumpTable, 0x00400000, MEM_COMMIT, PAGE_READWRITE)==NULL) {
		DisplayError("Not enough memory for JumpTable!");
		return FALSE;
	}
